...
// 3. pickModel возвращает корректный объект модели.
check("pickModel(0) → самая сильная модель", pickModel(0).id === MODEL_TIERS[0].id);
check("pickModel(big) → самая слабая модель", pickModel(999999).id === MODEL_TIERS.at(-1).id);
