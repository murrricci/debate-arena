...
табло показывает результаты. Раньше настройки агентов и очки жили **только в localStorage
браузера киоска** (`src/lib/store.js`), а бэкенд (`server.js`) был stateless-прокси к LLM.
...
- `store.js` превращается в тонкий API-клиент с синхронным in-memory `cache`; `localStorage` используется только как одноразовый источник миграции:
...
- **API недоступен**: фронт debate-arena сохраняет только последний in-memory snapshot текущей сессии и не перечитывает `localStorage`; bot-conf-max показывает баннер, читает кэш `bot.arena_agents`, форма read-only, сохранение заблокировано с понятным текстом.
